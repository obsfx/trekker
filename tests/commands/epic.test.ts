...
describe('epic command', () => {
  ...
  describe('create', () => {
    ...
    it('should create an epic with all options', () => {
      const epic = ctx.runToon<Epic>(
        'epic create -t "Full Epic" -d "Description here" -p 1 -s in_progress'
      );
  ...
  describe('list', () => {
    ...
    it('should filter by status', () => {
      ...
      const todoResult = ctx.runToon<PaginatedResponse<Epic>>('epic list -s todo');
      expect(todoResult.items).toHaveLength(1);
      expect(todoResult.items[0].title).toBe('Todo Epic');

      const inProgressResult = ctx.runToon<PaginatedResponse<Epic>>('epic list -s in_progress');
      expect(inProgressResult.items).toHaveLength(1);
      expect(inProgressResult.items[0].title).toBe('In Progress Epic');
    });
  ...
  describe('update', () => {
    ...
    it('should update multiple fields at once', () => {
      const created = ctx.runToon<Epic>('epic create -t "Test"');
      const updated = ctx.runToon<Epic>(
        `epic update ${created.id} -t "Updated" -d "New desc" -s completed -p 1`
      );
