...
describe('task command', () => {
  ...
  describe('create', () => {
    ...
    it('should create a task with all options', () => {
      ...
      const task = ctx.runToon<Task>(
        `task create -t "Full Task" -d "Description" -p 1 -s in_progress --tags "tag1,tag2" -e ${epic.id}`
      );
  ...
  describe('list', () => {
    it('should list all tasks', () => {
      ...
      const result = ctx.runToon<PaginatedResponse<Task>>('task list');
    ...
    it('should filter by status', () => {
      ...
      const todoResult = ctx.runToon<PaginatedResponse<Task>>('task list -s todo');
      expect(todoResult.items).toHaveLength(1);
      expect(todoResult.items[0].title).toBe('Todo Task');

      const inProgressResult = ctx.runToon<PaginatedResponse<Task>>('task list -s in_progress');
      expect(inProgressResult.items).toHaveLength(1);
      expect(inProgressResult.items[0].title).toBe('In Progress Task');
    });
    ...
    it('should filter by epic', () => {
      ...
      const epic1Result = ctx.runToon<PaginatedResponse<Task>>(`task list -e ${epic1.id}`);
      expect(epic1Result.items).toHaveLength(1);
      expect(epic1Result.items[0].title).toBe('Task in Epic 1');
    });
    ...
    it('should not list subtasks', () => {
      ...
      expect(result.items[0].title).toBe('Parent Task');
    });
  ...
  describe('update', () => {
    ...
    it('should update multiple fields at once', () => {
      const created = ctx.runToon<Task>('task create -t "Test"');
      const updated = ctx.runToon<Task>(
        `task update ${created.id} -t "Updated" -d "New desc" -s completed -p 1 --tags "a,b"`
      );
