...
describe('comment command', () => {
  ...
  describe('add', () => {
    ...
    it('should add multiple comments to the same task', () => {
      ...
      const result = ctx.runToon<PaginatedResponse<Comment>>(`comment list ${task.id}`);
    ...
    it('should handle multiline content', () => {
      const comment = ctx.runToon<Comment>(
        `comment add ${task.id} -a "agent" -c "Line 1\\nLine 2\\nLine 3"`
      );
  ...
  describe('list', () => {
    ...
    it('should only list comments on specified task', () => {
      ...
      expect(result.items[0].content).toBe('Comment on task 1');
    });
  });
