...
describe('subtask command', () => {
  ...
  describe('create', () => {
    ...
    it('should create a subtask with all options', () => {
      const subtask = ctx.runToon<Task>(
        `subtask create ${parentTask.id} -t "Full Subtask" -d "Description" -p 1 -s in_progress`
      );
    ...
    it('should fail with invalid status', () => {
      const error = ctx.runExpectError(
        `subtask create ${parentTask.id} -t "Test" -s invalid_status`
      );
  ...
  describe('list', () => {
    it('should list all subtasks of a parent task', () => {
      ...
      const result = ctx.runToon<PaginatedResponse<Task>>(`subtask list ${parentTask.id}`);
  ...
  describe('update', () => {
    ...
    it('should update multiple fields at once', () => {
      ...
      const updated = ctx.runToon<Task>(
        `subtask update ${subtask.id} -t "Updated" -d "New desc" -s completed -p 1`
      );
