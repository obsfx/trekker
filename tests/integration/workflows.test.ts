...
describe('workflow integration tests', () => {
  ...
  describe('full task lifecycle', () => {
    it('should handle create -> update -> comment -> complete workflow', () => {
      ...
      ctx.run(
        `comment add ${task.id} -a "agent" -c "Completed: Login form with validation implemented"`
      );
      ...
      // Verify comments
      const comments = ctx.runToon<PaginatedResponse<Comment>>(`comment list ${task.id}`);
  ...
  describe('epic workflow', () => {
    it('should complete epic and archive all tasks and subtasks', () => {
      ...
      ctx.runToon<Task>(`task create -t "Design auth flow" -e ${epic.id}`);
      ...
      // Complete the epic
      const result = ctx.runToon<{ epic: string; archived: { tasks: number; subtasks: number } }>(
  ...
  describe('search functionality', () => {
    it('should find entities across all types', () => {
      ...
      const task = ctx.runToon<Task>(
        `task create -t "Auth task" -d "Implement authentication" -e ${epic.id}`
      );
  ...
  describe('list and filter integration', () => {
    ...
    it('should sort and paginate correctly', () => {
      ...
      const page1 = ctx.runToon<ListResponse>(
        'list --type task --sort priority:asc --limit 5 --page 1'
      );
      const page2 = ctx.runToon<ListResponse>(
        'list --type task --sort priority:asc --limit 5 --page 2'
      );
  ...
  describe('data integrity', () => {
    it('should maintain consistent state after multiple operations', () => {
      ...
      const epicTasks = ctx.runToon<PaginatedResponse<Task>>(`task list -e ${epic.id}`);
      expect(epicTasks.items).toHaveLength(5);
